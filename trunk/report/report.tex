...
The main goal of the system is to be as flexible as possible, giving the
possibility to attach an arbitrary number of annotations (which we call labels)
to a sequence, to have different kinds of labels, to enable automatic label generation when
a sequence is added into the system or is modified, and mainly,
to make arbitrarily complex search queries using those annotated labels.
Efficient search of sequences with some specific characteristics is then,
the main objective of the system and its crucial functionality.

When searching for a certain set of sequences the user will
also be able to apply operations to the whole sequence list,
such as: add new labels, delete current labels, change label
values, export the sequence list and get the translated sequence
list, that is, get the related protein sequences from a list of DNA sequences.

Each label stored in the system has a certain number of properties that define
the label behavior. These properties will indicate if the the label should be
auto generated when a sequence is added, if it is user editable or deletable.
More importantly, the system must store, if that is the case, the code used to generate default label values.

The system will support basic label types such as: integer, float, boolean,
...
Another important facility that must be present in the system is the use of
taxonomies, supporting the storing of various taxonomy trees and a specific
label type that is a reference to a taxonomy. The basic use case of this
functionality is the creation of a label named 'species' and then attaching
this 'species' label to specific sequences.

The NCBI taxonomy tree is present by default, but the system allows the creation of new and custom taxonomy trees.

Every data stored should also be exportable and importable, to and from files.
The system accepts the FASTA and XML formats to export sequences and related annotations.
...
Some clients can be application administrators, given them the permission to edit user information, manage labels and do database maintenance.
...
integrated search environment over a set of sequences, using the generated sequences.
...
Update and insert triggers were also put in place to automatically update time information.
...
\begin{itemize}
  ...
  \begin{itemize}
    \item id - primary index.
    \item history\_id - foreign key index.
    \item content - index with size 15 to do faster sequence searches using content.
    \item name - index with size 10 to locate sequences by name.
  \end{itemize}
...
number of sequences for a specific query. When we know the total number of rows in the result,
we can then fetch partial results using the \textbf{get\_search} method.
  ...
  \Begin{
    ...
    \ElseIf{query is terminal}{
      ...
      \ElseIf{label is standard}{
        ...
        \Return "EXISTS(SELECT label\_sequence\_info.id FROM label\_sequence\_info WHERE label\_sequence\_info.seq\_id = sequence\_info\_history.id AND label\_sequence\_info.label\_id = " + label\_id + " AND " + field + " " + oper + " " + value + param\_sql + ")"\;
      }
    }
  }
...
FROM  (SELECT id AS orig_id FROM sequence_info_history WHERE $sql_where) all_seqs
      NATURAL JOIN
      (SELECT seq_id AS orig_id, ref_data AS id FROM label_sequence WHERE label_id = $transform_label
...
meganon, paup and paupon formats. Remaining system objects can only be exported
in the XML format.
...
It is structured into three parts: query input, operations and results.
...
It is also possible to select parts of query in the tree to know how many results
there are for that query sub-expression.
...
and forth between the search page and each operations page, without losing any work. 
...
Appearing right below the query input component, the operator component is shown in Figure \ref{fig:search2}.
...
The preview component shows all the sequences that match the query in a grid component.
...
Optionally we can also transform the results using the \textbf{Transform results} option and
selecting a reference label. Everything is shown in Figure \ref{fig:search3}.
...
In this page (Figure \ref{fig:sequence_upload}) three options are available: upload a single file; upload both DNA and protein file, linking
...
For space measurements, we compute the database size using the SQL query presented in Listing.
...
Initial database size was 247267KB.
...
\begin{table}[H]
  \begin{center}
    ...
    \begin{tabular}{ | l | c | c | c | c | }
      ...
      (1) big.fasta with no protein translation & 0 & 1m2.931s & 249795KB & 2528KB \\ \hline
      (2) big.fasta with protein translation & 0 & 2m1.767s & 250355KB & 3088KB \\ \hline
      (3) serpina1.dna.fasta + serpina1.trans.fasta & 0 & 0m2.497s & 247347KB & 80KB \\ \hline
      (4) big.fasta with protein translation & 748: test (2) results & 0m56.399s & 251379KB & 4112KB \\ \hline
      (5) serpina1.dna.fasta + serpina1.trans.fasta & 748: test (2) results & 0m2.612s & 251939KB & 4672KB \\ \hline
      (6) serpina1.dna.fasta + serpina1.trans.fasta & 778: test (5) results & 0m0.593s & 251939KB & 4672KB \\ \hline
    \end{tabular}}
  \end{center}
...
Another conclusion we can make is that the import time is greatly reduced when the sequences are already in the system.
...
\begin{table}[H]
  \begin{center}
    ...
    \begin{tabular}{ | l | c | c | c | c | }
      ...
      (1) annotate all sequences with \textbf{url} (google.pt, parameter google) & 778 & 0m7.913s & 253091KB & 1152KB \\ \hline
      (2) annotate all sequences with \textbf{url} (sapo.pt, parameter sapo) & 778: test (1) & 0m8.135s & 253155KB & 1216KB \\ \hline
      (3) annotate all sequences with \textbf{refpos} (start: 1, length: 10) & 778: test (2) & 0m8.029s & 253331KB & 1392KB \\ \hline
      (4) edit all \textbf{refpos} instances (start: 2, length: 10) & 778: test (3) & 0m8.656s & 253331KB & 1392KB \\ \hline
    \end{tabular}}
  \end{center}
...
(404 sequences).
Then we ran some test expressions against the set, then we augmented the set with big.fasta protein translations and ran
the same test expressions and everything was timed.

\begin{table}[H]
    \begin{tabular}{ | p{11.2cm} | c | c |}
      \hline
      \textbf{Expression} & \textbf{Results} & \textbf{Time} \\ \hline
      length exists & 404 & 0m0.371s \\ \hline
      length $>$ 0 & 404 & 0m0.399s \\ \hline
      length $>$ 0 and name regexp A & 47 & 0m0.157s \\ \hline
      length $>$ 0 and name regexp A and content regexp A & 47 & 0m0.158s \\ \hline
      length $>$ 0 and name regexp A and content regexp A and species like Drosophila & 43 & 0m8.668s \\ \hline
  \end{tabular}
...
\begin{table}[H]
    \begin{tabular}{ | p{11.2cm} | c | c |}
      \hline
      \textbf{Expression} & \textbf{Results} & \textbf{Time} \\ \hline
      length exists & 30 & 0m0.135s \\ \hline
      length $>$ 0 & 30 & 0m0.209s \\ \hline
      length $>$ 0 and name regexp A & 4 & 0m0.136s \\ \hline
      length $>$ 0 and name regexp A and content regexp A & 4 & 0m0.146s \\ \hline
      length $>$ 0 and name regexp A and content regexp A and species like Drosophila & 0 &  0m8.951s \\ \hline
  \end{tabular}
...
\begin{table}[H]
    \begin{tabular}{ | p{11.2cm} | c | c |}
      \hline
      \textbf{Expression} & \textbf{Results} & \textbf{Time} \\ \hline
      length exists & 778 & 0m0.526s \\ \hline
      length $>$ 0 & 778 & 0m0.323s \\ \hline
      length $>$ 0 and name regexp A & 90 & 0m0.183s \\ \hline
      length $>$ 0 and name regexp A and content regexp A & 90 & 0m0.179s \\ \hline
      length $>$ 0 and name regexp A and content regexp A and species like Drosophila & 43 & 0m9.628s \\ \hline
  \end{tabular}
  \caption{Searching 778 sequences with no translation.}
...
\begin{table}[H]
    \begin{tabular}{ | p{11.2cm} | c | c |}
      \hline
      \textbf{Expression} & \textbf{Results} & \textbf{Time} \\ \hline
      length exists & 778 & 0m0.371s \\ \hline
      length $>$ 0 & 778 & 0m0.401s \\ \hline
      length $>$ 0 and name regexp A & 90 & 0m0.548s \\ \hline
      length $>$ 0 and name regexp A and content regexp A & 90 & 0m0.217s \\ \hline
      length $>$ 0 and name regexp A and content regexp A and species like Drosophila & 43 & 0m8.235s \\ \hline
  \end{tabular}
...
The performance also decreases when the search expression gets more complex.

\clearpage
\chapter{Conclusion}

In this technical report we presented a possible design and implementation of a bioinformatics
system that was built to store sequences and annotations, which we called labels.
...
\begin{thebibliography}
  ...
  \bibitem{cbs} Peter F. Hallin and David W. Ussery. CBS Genome Atlas Database: a dynamic storage for bioinformatic results and sequence data. \textit{Bioinformatics} Volume 20 Issue 18, 2004.
	...
	\bibitem{extensible_open_source} Sean D. Mooney and Peter H. Baenziger. Extensible open source content management systems and frameworks: a solution for many needs of a bioinformatics group. \textit{Center for Computational Biology and Bioinformatics, Department of Medical and Molecular Genetics, Indiana University School of Medicine}, 2007.
	  ...
	  \bibitem{gendb} Folker Meyer, Alexander Goesmann, Alice C. McHardy, Daniela Bartels, Thomas Bekel, Jorn Clausen, Jorn Kalinowski, Burkhard Linke, Oliver Rupp, Robert Giegerich and Alfred Puhler. GenDB: an open source genome annotation system for prokaryote genomes. \textit{Nucleic Acids Research}, 2003.
...
\begin{table}[H]
  \begin{tabular}{ | c | p{12cm} |}
    ...
    Sequence & \begin{itemize}
      \item id (SERIAL)
      \item content (TEXT): The sequence content.
      \item name (VARCHAR)
...
\begin{table}[H]
  \begin{tabular}{ | >{\centering}m{20mm} | p{12cm} |}
    ...
    File & \begin{itemize}
      ...
      \item name (VARCHAR(512)): File name.
